...
(function () {
  ...
  '<header class="mc-header">' +
    '<div class="mc-head">' +
      ...
      '<a href="/" class="mc-logo"><img src="assets/logo.png" alt="My Candy\'s" width="160" height="54"></a>' +
      '<div class="mc-actions">' +
        '<button id="mc-search-btn" class="mc-act" title="Recherche"><span>🔍</span><span class="mc-lbl">Recherche</span></button>' +
        '<a href="compte" class="mc-act" data-account title="Mon compte"><span>👤</span><span class="mc-lbl">Compte</span></a>' +
        '<a href="favoris" class="mc-act" title="Favoris"><span>🤍</span><span class="mc-lbl">Favoris</span><span id="wish-badge" class="mc-wbadge">0</span></a>' +
    ...
    '<nav class="mc-navwrap"><div class="mc-row mc-nav">' +
      ...
      '<div class="mc-drop">' +
        ...
        '<div class="mc-dropmenu" style="min-width:640px">' +
          '<div class="mc-dropcol"><div>🍬 Bonbons</div><a href="boutique/rayon/barbeapapa">Barbe à papa</a><a href="boutique/Bonbons">Bonbons US</a><a href="boutique/rayon/gum">Chewing-gum &amp; sucettes</a><a href="boutique/Bonbons" class="mc-seeall">Voir tout les bonbons →</a></div>' +
          '<div class="mc-dropcol"><div>🍿 Snacks sucrés</div><a href="boutique/rayon/biscuit">Biscuits</a><a href="boutique/rayon/cereales">Céréales</a><a href="boutique/rayon/jelly">Gelées</a><a href="boutique/rayon/mochi">Mochi</a><a href="boutique/rayon/popcorn">Pop-corn</a><a href="boutique/Desserts" class="mc-seeall">Voir tout les desserts →</a></div>' +
          '<div class="mc-dropcol"><div>🍫 Chocolats</div><a href="boutique/rayon/barres">Barres chocolatées</a><a href="boutique/rayon/peanut">Beurre de cacahuète</a><a href="boutique/rayon/dubai">Chocolat de Dubaï ✨</a><a href="boutique/Chocolats">Chocolats intl</a><a href="boutique/Chocolats" class="mc-seeall">Voir tout les chocolats →</a></div>' +
        '</div>' +
      '</div>' +
      '<div class="mc-drop">' +
        '<a href="boutique/sales" class="mc-navlink' + act('sales') + '">Salés ▾</a>' +
        '<div class="mc-dropmenu" style="min-width:420px">' +
          '<div class="mc-dropcol"><div>🌶️ Snacks salés</div><a href="boutique/rayon/jerky">Beef jerky</a><a href="boutique/rayon/chips">Chips &amp; crackers</a><a href="boutique/rayon/takis">Takis &amp; snacks épicés</a><a href="boutique/sales" class="mc-seeall">Voir tout →</a></div>' +
          '<div class="mc-dropcol"><div>🍜 Nouilles &amp; salsa</div><a href="boutique/rayon/nouilles">Nouilles</a><a href="boutique/rayon/salsa">Salsa</a></div>' +
        '</div>' +
      '</div>' +
  ...
  var MENU = [
    ...
    { t: 'Sucrés', h: 'boutique/sucres', all: 'Tous les produits sucrés', sub: [
      ['Barbe à papa', 'boutique/rayon/barbeapapa'], ['Barres chocolatées', 'boutique/rayon/barres'], ['Beurre de cacahuète', 'boutique/rayon/peanut'], ['Biscuits', 'boutique/rayon/biscuit'], ['Bonbons US', 'boutique/Bonbons'], ['Céréales', 'boutique/rayon/cereales'], ['Chewing-gum & sucettes', 'boutique/rayon/gum'], ['Chocolat de Dubaï ✨', 'boutique/rayon/dubai'], ['Chocolats intl', 'boutique/Chocolats'], ['Gelées & gummies', 'boutique/rayon/jelly'], ['Mochi', 'boutique/rayon/mochi'], ['Pop-corn', 'boutique/rayon/popcorn'] ] },
    { t: 'Salés', h: 'boutique/sales', all: 'Tous les produits salés', sub: [
      ['Beef jerky', 'boutique/rayon/jerky'], ['Chips & crackers', 'boutique/rayon/chips'], ['Nouilles', 'boutique/rayon/nouilles'], ['Salsa', 'boutique/rayon/salsa'], ['Takis & snacks épicés', 'boutique/rayon/takis'] ] },
  ...
  '<aside id="mc-menu" class="mc-drawer">' +
    '<div class="mc-menu-head"><a href="/"><img src="assets/logo.png" alt="My Candy\'s"></a><button id="mc-menu-close" class="mc-menu-close" title="Fermer">✕</button></div>' +
    mdrawer() +
    '<div class="mc-menu-foot"><a href="compte" data-account><span>👤</span> Compte</a><a href="favoris"><span>🤍</span> Favoris</a></div>' +
  '</aside>' +
  ...
  '<aside id="mc-cart" class="mc-drawer">' +
    ...
    '<div class="mc-cart-foot" id="mc-cart-foot">' +
      ...
      '<a href="checkout" class="mc-cart-pay">Passer au paiement →</a>' +
  ...
  (function () {
    ...
    function writeU(u) { try { localStorage.setItem(UKEY, JSON.stringify(u)); } catch (e) {} try { window.dispatchEvent(new Event('mc-auth-change')); } catch (e) {} }
    function clearU() { try { localStorage.removeItem(UKEY); } catch (e) {} try { window.dispatchEvent(new Event('mc-auth-change')); } catch (e) {} }
    ...
    /* Reflète l'état « connecté » dans le header (avatar + prénom) sur TOUTES les pages, en permanence. */
    function paintChrome() {
      var u = readU();
      var on = !!(u && (u.email || u.provider));
      var ini = on ? (((u.firstName || '').trim().charAt(0)) || '🍬').toUpperCase() : '';
      var name = on ? (u.firstName || 'Mon compte') : 'Compte';
      var hb = document.querySelector('.mc-act[data-account]');
      if (hb) {
        var ic = hb.querySelector('span'), lb = hb.querySelector('.mc-lbl');
        if (ic) { ic.textContent = on ? ini : '👤'; ic.classList.toggle('mc-avai', on); }
        if (lb) lb.textContent = name;
        hb.setAttribute('title', on ? ('Mon compte — ' + name) : 'Mon compte');
      }
      var mf = document.querySelector('.mc-menu-foot a[data-account]');
      if (mf) mf.innerHTML = '<span' + (on ? ' class="mc-avai"' : '') + '>' + (on ? ini : '👤') + '</span> ' + name;
    }
    paintChrome();
    window.addEventListener('mc-auth-change', paintChrome);
    window.addEventListener('storage', function (e) { if (e.key === UKEY) paintChrome(); });
...
/* ---------- Bandeau cookies (consentement simple : essentiels + contenus tiers / mesure d'audience) ---------- */
(function () {
  var CK = 'mcCookieV1';
  function get() { try { return localStorage.getItem(CK); } catch (e) { return null; } }
  window.MCConsent = get();
  window.MCcanTrack = function () { return get() === 'yes'; };
  function set(v) {
    try { localStorage.setItem(CK, v); } catch (e) {}
    window.MCConsent = v;
    try { window.dispatchEvent(new Event('mc-consent-change')); } catch (e) {}
  }
  window.MCsetConsent = set;
  if (get() !== null) return; // choix déjà fait → pas de bandeau
  function build() {
    if (document.getElementById('mc-cookie')) return;
    ...
    function done(v) { set(v); if (el.parentNode) el.parentNode.removeChild(el); }
    document.getElementById('mc-cookie-yes').addEventListener('click', function () { done('yes'); });
    document.getElementById('mc-cookie-no').addEventListener('click', function () { done('no'); });
  }
